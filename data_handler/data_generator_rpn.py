...
"""
the module to generator data for training rpn_network & Fast-RCNN
"""


class data_generator_rpn(generator):
    def __init__(self):
        ...
        # self.network = resnet()
        super().__init__('train',resnet())

    # @property
    # def step_per_epoch(self):
    #     return self._step_epoch
    def handle(self, image, gt_box):
        """
            ...
            for j in range(iou_record.shape[1]):
                for x in range(iou_record.shape[2]):
                    if iou_record[i,j,x]==best_iou and box_infos[i,j,2*x]==1:
                        ...
                        # If there are no anchor box's IoU value greater than threshold, get the max value.
